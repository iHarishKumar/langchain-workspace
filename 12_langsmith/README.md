...
🏁 **That's the series.** Twelve patterns, one arc: `invoke` → chains → parallel → routing → orchestration → evaluation loops → agents → memory → RAG → multi-agent → performance → observability. The [root README](../README.md) has the full map — and the suggested paths back through it.
